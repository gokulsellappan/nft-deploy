...
contract ERC1155{
    ...
    event TransferBatch(address indexed _operator,address indexed _from,address indexed _to,uint256[] _id,uint256[] _value);
    ...
    function safeTransferFrom(address from,address to,uint id,uint256 amount)public virtual{
        require(from==msg.sender||isApprovedForAll(from,msg.sender),"msg.sender is not owner or approved for transfer");
    ...
    function safeBatchTranferFrom(address from,address to,uint256[] memory ids,uint256[] memory amounts)public{
        require(from==msg.sender||isApprovedForAll(from,msg.sender),"msg.sender is not owner or approved for transfer");
        require(to!=address(0),"address is not 0");
